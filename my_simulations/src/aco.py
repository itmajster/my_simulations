...
class Map:
    ...
    class Nodes:        #class for representing the nodes
        def __init__(self, row, col, in_map,spec):
            self.node_pos= (row, col)
            self.edges = self.compute_edges(in_map)
            self.spec = spec

        def compute_edges(self,map_arr):        #function for creating and connecting the edges to nodes
...
class AntColony:
    ...
    class Ant:
        ...
        def move_ant(self, node_to_visit):          #moving ant to selected node
            self.actual_node = node_to_visit
            self.remember_visited_node(node_to_visit)   

        def remember_visited_node(self, node_pos):  #appends visited nodes to the list
            self.visited_nodes.append(node_pos)

        def get_visited_nodes(self):                #returns the list of visited nodes
            return self.visited_nodes

        def is_final_node_reached(self):            #checks if the ant is in final node
            if self.actual_node == self.final_node :
                self.final_node_reached = True

        def enable_start_new_path(self):            #uncheck the state of ant being in the final node
            self.final_node_reached = False         #to being able to start over

        def setup_ant(self):                        #clears the list of visited nodes
            self.visited_nodes[1:] =[]
            self.actual_node= self.start_pos        #set first node as initial
    ...
    def create_ants(self):                          #creating the ants 
        ants = []
        for i in range(self.no_ants):
            ants.append(self.Ant(self.map.initial_node, self.map.final_node))
        return ants

    def select_next_node(self, actual_node):        #randomly selecting the next node
    ...
    def pheromone_update(self):         #updating the pheromone levels and sorting paths by leng
    ...
    def empty_paths(self):      #empty the list of paths
        self.paths[:]

    def sort_paths(self):       #sorting the paths
        self.paths.sort(key=len)

    def add_to_path_results(self, in_path):         #appends path to the list
        self.paths.append(in_path)

    def get_coincidence_indices(self,lst, element): #coincidence indices of elements in path
        result = []
        offset = -1
        while True:
            try:
    ...
    def delete_loops(self, in_path):    #if loops in the path -> delete them
    ...
    def calculate_path(self):       #handling the process of finding the best path
...
def main():
    ...
    if maze[start[0]][start[1]] == 0 or maze[end[0]][end[1]] == 0:      #check if the start and end nodes are valid
