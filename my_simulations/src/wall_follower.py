...
def go_to_goal(xx, yy):
	...
	while stop_stamp == 0:
		...
		#after reaching desired node, recompute and adjust the angles of robot rotation according to the current heading value
		else:
			stop_stamp = 1
			if(heading=='e'):
...
#function to evaluate next node according to current heading=east
...
#function to evaluate next node according to current heading=south
...
#function to evaluate next node according to current heading=west
...
#function to evaluate next node according to current heading=north
...
'''
	...
	SUBSCRIBER FOR HANDLING LASER DATA
...
pub = rospy.Publisher('/cmd_vel', Twist, queue_size=1)	#velocity publisher
...
def main():
	...
	front = 0
	left = 0
	wall_found = 0		#found the wall? 0 = not found; 1 = found
	a = 0.75  			# maximum threshold distance
	path_done = 0		#stamp for path done...0 = not in the goal...1 = in the goal

	#initialize start node and check if start or end node is either in the map or valid empty node
	tmp_maze = maze[::-1]    
	...
	#initialize empty array for result path
	path = []
	path.append(start)
